...
* la gestion des recettes, d’une grille de menus unique par séjour, des régimes
  alimentaires et des quantités individuelles par public cible ;
* la gestion des denrées, fournisseurs, références et conditionnements ;
* la saisie et la consultation des mouvements de stock ;
* la distribution Scout Market classique avec gestion des repas Explo,
  pique-nique et non pris ;
* le calcul des commandes sur une plage de repas avec déduction du stock ;
...
État vérifié le 2 septembre 2026 : nouvelle base applicative `v1.0.0`, dérivée
du code stable `v1.4.1`. L’historique Git et le schéma Liquibase repartent de
zéro. Le changeset unique `V001` est destiné exclusivement aux installations
neuves et n’est pas compatible avec une mise à jour directe d’une ancienne
base Campement. La production existante reste hors du périmètre de cette
séparation et ne doit pas être modifiée.
...
│       │   └── V001__initialisation_campement.sql
│       └── dev/              # données réservées au développement et aux tests
│           └── D001__jeu_donnees_developpement.sql
└── app/
