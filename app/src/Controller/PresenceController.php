...
final class PresenceController extends AbstractController
{
    ...
    public function index(Request $request, ContexteSejour $contexte, GroupeRepository $groupes, ParticipantRepository $participants, EntityManagerInterface $em): Response
    {
        ...
        $erreurs = [];
        $routeEdition = 'app_presence_modifier' === $request->attributes->get('_route');
        if ($request->isMethod('POST')) {
            if (!$this->isCsrfTokenValid('modifier_presence', $request->request->getString('_token'))) {
            ...
            if (!$participant instanceof Participant || $participant->getGroupe()->getSejour() !== $sejour) {
                $erreurs[] = 'Participant invalide.';
            }
            if (!$date || $date < $participant?->getDateDebutPresence() || $date > $participant?->getDateFinPresence()) {
                $erreurs[] = 'Date de présence invalide.';
            }
            if (!in_array($statut, ['present', PresenceParticipant::ABSENT, PresenceParticipant::DEPART], true)) {
                $erreurs[] = 'Statut invalide.';
            }
            if (PresenceParticipant::DEPART === $statut && '' === $commentaire) {
                $erreurs[] = 'Le commentaire est obligatoire pour un départ.';
            }
            ...
            if ([] === $erreurs) {
                ...
                if (in_array($statut, ['present', PresenceParticipant::ABSENT], true) && $departPrecedent && $departPrecedent !== $presence) {
                    $em->remove($departPrecedent);
                }
                if ('present' === $statut) {
                    if ($presence) {
                        $em->remove($presence);
                    ...
                    $presence ??= (new PresenceParticipant())->setParticipant($participant)->setDatePresence($date);
                    $presence->setStatut($statut)->setCommentaire('' === $commentaire ? null : $commentaire)->actualiser();
                    $em->persist($presence);
                }
                ...
                $em->flush();
                $this->addFlash('success', 'Le registre de présence a été mis à jour.');

                return $this->redirectToRoute('app_presence', ['date' => $date->format('Y-m-d')]);
            }
        }
        if ($routeEdition) {
            $id = $request->isMethod('POST') ? $request->request->getString('participant_id') : (string) $request->attributes->get('participantId');
            $dateTexte = $request->isMethod('POST') ? $request->request->getString('date') : (string) $request->attributes->get('date');
            $participant = Uuid::isValid($id) ? $participants->find($id) : null;
            $date = \DateTimeImmutable::createFromFormat('!Y-m-d', $dateTexte);
            if (!$participant instanceof Participant || $participant->getGroupe()->getSejour() !== $sejour || !$date || $date < $participant->getDateDebutPresence() || $date > $participant->getDateFinPresence()) {
                throw $this->createNotFoundException('Présence introuvable.');
            }
            $presence = $em->getRepository(PresenceParticipant::class)->findOneBy(['participant' => $participant, 'datePresence' => $date]);
            $statut = $request->isMethod('POST') ? $request->request->getString('statut') : ($presence?->getStatut() ?? 'present');
            $commentaire = $request->isMethod('POST') ? $request->request->getString('commentaire') : ($presence?->getCommentaire() ?? '');

            return $this->render('presence/modifier.html.twig', compact('participant', 'date', 'statut', 'commentaire', 'erreurs'));
        }
        $liste = $participants->findPourSejour($sejour);
        $presences = $em->getRepository(PresenceParticipant::class)->createQueryBuilder('p')->join('p.participant', 'participant')->join('participant.groupe', 'groupe')->where('groupe.sejour=:sejour')->setParameter('sejour', $sejour)->getQuery()->getResult();
        $index = [];
        foreach ($presences as $p) {
            $index[(string) $p->getParticipant()->getId()][$p->getDatePresence()->format('Y-m-d')] = $p;
        }
        ...
        $groupesDuJour = $groupes->findActifsPresentsPourSejour($sejour, $dateSelection);
        $groupesAffiches = [];
        foreach ($groupesDuJour as $groupe) {
            $groupesAffiches[(string) $groupe->getId()] = true;
        }
        $jour = [];
        $totauxPresents = ['jeunes' => 0, 'adultes' => 0];
        $key = $dateSelection->format('Y-m-d');
        foreach ($liste as $participant) {
            $exception = $index[(string) $participant->getId()][$key] ?? null;
            $depart = null;
            foreach ($index[(string) $participant->getId()] ?? [] as $candidate) {
                if (PresenceParticipant::DEPART === $candidate->getStatut() && $candidate->getDatePresence() <= $dateSelection) {
                    $depart = $candidate;
                    break;
                }
            }
            $outside = $dateSelection < $participant->getDateDebutPresence() || $dateSelection > $participant->getDateFinPresence();
            $statut = $outside ? 'outside' : ($depart ? 'departed' : (PresenceParticipant::ABSENT === $exception?->getStatut() ? 'absent' : 'present'));
            $jour[(string) $participant->getId()] = ['statut' => $statut, 'modifiable' => !$outside, 'exception' => $exception ?? $depart];
            if ('present' === $statut && isset($groupesAffiches[(string) $participant->getGroupe()->getId()])) {
