...
final class DuplicationSejour
{
    ...
    public function dupliquer(Sejour $source, Sejour $cible, array $choix, Utilisateur $auteur): void
    {
        ...
        $s = (string) $source->getId(); $c = (string) $cible->getId();
        $this->connexion->transactional(function (Connection $db) use ($s, $c, $choix, $besoinDenrees, $besoinFournisseurs, $auteur): void {
            if ($besoinFournisseurs) $db->executeStatement('INSERT INTO campement.fournisseur (id,sejour_id,nom,telephone,email,adresse,actif) SELECT uuidv7(),:c,nom,telephone,email,adresse,actif FROM campement.fournisseur WHERE sejour_id=:s', compact('s','c'));
            if ($besoinDenrees) $db->executeStatement('INSERT INTO campement.denree (id,sejour_id,nom,unite_reference_id,unite_inventaire_id,stock_min,actif) SELECT uuidv7(),:c,nom,unite_reference_id,unite_inventaire_id,stock_min,actif FROM campement.denree WHERE sejour_id=:s', compact('s','c'));
            ...
            if (in_array('recettes', $choix, true)) {
                ...
                $db->executeStatement('INSERT INTO campement.recette_denree (id,recette_id,denree_id,conditionnement_id,ordre) SELECT uuidv7(),rc.id,dc.id,rd.conditionnement_id,rd.ordre FROM campement.recette_denree rd JOIN campement.recette rs ON rs.id=rd.recette_id JOIN campement.recette rc ON rc.sejour_id=:c AND rc.nom=rs.nom JOIN campement.denree ds ON ds.id=rd.denree_id JOIN campement.denree dc ON dc.sejour_id=:c AND dc.nom=ds.nom WHERE rs.sejour_id=:s', compact('s','c'));
            ...
            if (in_array('menus', $choix, true)) {
                ...
                $db->executeStatement('INSERT INTO campement.menu_denree (id,menu_id,denree_id,conditionnement_id,categorie,ordre) SELECT uuidv7(),mc.id,dc.id,md.conditionnement_id,md.categorie,md.ordre FROM campement.menu_denree md JOIN campement.menu ms ON ms.id=md.menu_id JOIN campement.menu mc ON mc.sejour_id=:c AND mc.special_code IS NOT DISTINCT FROM ms.special_code AND mc.date_menu IS NOT DISTINCT FROM (ms.date_menu + ((SELECT date_debut FROM campement.sejour WHERE id=:c)-(SELECT date_debut FROM campement.sejour WHERE id=:s))) AND mc.sejour_type_repas_id IS NOT DISTINCT FROM (SELECT tc.id FROM campement.sejour_type_repas ts JOIN campement.sejour_type_repas tc ON tc.sejour_id=:c AND tc.type_repas_id=ts.type_repas_id WHERE ts.id=ms.sejour_type_repas_id) JOIN campement.denree ds ON ds.id=md.denree_id JOIN campement.denree dc ON dc.sejour_id=:c AND dc.nom=ds.nom WHERE ms.sejour_id=:s', compact('s','c'));
                $db->executeStatement('INSERT INTO campement.menu_denree_quantite (id,menu_denree_id,sejour_public_cible_id,quantite_individuelle) SELECT uuidv7(),mdc.id,pcc.id,q.quantite_individuelle FROM campement.menu_denree_quantite q JOIN campement.menu_denree mds ON mds.id=q.menu_denree_id JOIN campement.menu ms ON ms.id=mds.menu_id JOIN campement.denree ds ON ds.id=mds.denree_id JOIN campement.menu mc ON mc.sejour_id=:c AND mc.special_code IS NOT DISTINCT FROM ms.special_code AND mc.date_menu IS NOT DISTINCT FROM (ms.date_menu + ((SELECT date_debut FROM campement.sejour WHERE id=:c)-(SELECT date_debut FROM campement.sejour WHERE id=:s))) JOIN campement.denree dc ON dc.sejour_id=:c AND dc.nom=ds.nom JOIN campement.menu_denree mdc ON mdc.menu_id=mc.id AND mdc.denree_id=dc.id AND mdc.ordre=mds.ordre JOIN campement.sejour_public_cible pcs ON pcs.id=q.sejour_public_cible_id JOIN campement.sejour_public_cible pcc ON pcc.sejour_id=:c AND pcc.public_cible_id=pcs.public_cible_id WHERE ms.sejour_id=:s', compact('s','c'));
            }
