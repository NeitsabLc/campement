...
class TacheSituationParticuliere
{
    ...
    public static function libre(SituationParticuliere $situation, string $libelle, ?\DateTimeImmutable $echeance = null): self
    {
        ...
        if ('' === $libelle || mb_strlen($libelle) > 200) {
            throw new \InvalidArgumentException('Le libellé libre est obligatoire et limité à 200 caractères.');
        }
    ...
    private function definirType(string $type): void
    {
        if (!isset(self::TYPES[$type])) {
            throw new \InvalidArgumentException('Le type de tâche est invalide.');
        }
        $this->typePredefini = $type;
    }

    public function getSituation(): SituationParticuliere
    {
        return $this->situation;
    }

    public function getTypePredefini(): ?string
    {
        return $this->typePredefini;
    }

    public function getLibelle(): string
    {
        return null !== $this->typePredefini ? self::TYPES[$this->typePredefini] : (string) $this->libelleLibre;
    }
    ...
    public function setDateEcheance(?\DateTimeImmutable $date): self
    {
    ...
    public function setStatut(string $statut, ?\DateTimeImmutable $dateRealisation = null): self
    {
        ...
        if (self::STATUT_REALISE === $statut && null === $dateRealisation) {
            $dateRealisation = new \DateTimeImmutable('today');
        }
        $this->statut = $statut;
