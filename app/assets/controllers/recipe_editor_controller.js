...
export default class extends Controller {
 ...
 add(){this.rowsTarget.insertAdjacentHTML('afterbegin',this.templateTarget.innerHTML);this.refresh();}
 remove(e){e.currentTarget.closest('.recipe-row').remove();this.refresh();}
 refresh(){[...this.rowsTarget.querySelectorAll('.recipe-row')].forEach((row,i)=>{const d=row.querySelector('[data-field="denree"]');const c=row.querySelector('[data-field="conditionnement"]');const selected=c.dataset.selected||c.value;c.replaceChildren(...(this.catalog[d.value]||[]).map(u=>new Option(u.nom,u.id,u.id===selected,u.id===selected)));delete c.dataset.selected;d.name=`lignes[${i}][denree]`;c.name=`lignes[${i}][conditionnement]`;row.querySelectorAll('[data-public]').forEach(x=>x.name=`lignes[${i}][quantites][${x.dataset.public}]`);});}
}
